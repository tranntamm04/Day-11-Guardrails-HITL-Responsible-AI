...
try:
    ...
    class _FallbackBasePlugin:
        """Small fallback so local output filter tests run without Google ADK."""
...
def content_filter(response: str) -> dict:
    ...
    PII_PATTERNS = {
        ...
        "connection_string": r"\b(?:postgresql|mysql|mongodb|redis)://\S+",
        "secret_phrase": r"\b(?:admin password|api key|secret|credential)s?\b[^.\n]*",
    }
...
class OutputGuardrailPlugin(base_plugin.BasePlugin):
    ...
    def _replace_text(self, llm_response, text: str):
        """Replace the response content with safe text."""
    ...
    async def after_model_callback(
        ...
        *,
        ...
        if not filter_result["safe"]:
            ...
            self._replace_text(llm_response, checked_text)
        ...
        if self.use_llm_judge:
            ...
            if not judge_result["safe"]:
                ...
                return self._replace_text(
                    llm_response,
                    "I cannot provide that information. I can help with "
                    "safe banking questions such as accounts, transfers, "
                    "payments, loans, savings, and cards.",
                )
